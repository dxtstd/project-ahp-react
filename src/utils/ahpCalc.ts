...
export function hitungAHP(dataKantin: TKantinMatrix[], matrixKriteria: number[][]) {
  ...
  if (dataKantin.length === 0) return { hasilRanking: [], crKriteria: 0 };
  ...
  dataKantin.forEach(k => {
    total.harga += k.harga; total.kebersihan += k.kebersihan; total.fasilitas += k.fasilitas;
    total.varian_menu += k.varian_menu; total.pelayanan += k.pelayanan;
  });
